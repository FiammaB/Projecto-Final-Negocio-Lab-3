...
export class PedidoService {
        ...
        try {
            ...
            for (const row of rows) {
                ...
                const detalles = detallesRows.map(det => {
                    const producto = new Producto(det.idproducto, det.codigoProducto, det.denominacion, det.precioVenta);
                    return new DetallePedido(det.id, det.idpedidoventa, det.idproducto, det.cantidad, det.subtotal, undefined, producto);
                });
                pedidos.push(new Pedido(row.pedidoId, row.idcliente, new Date(row.fechaPedido), row.nroComprobante, row.formaPago, row.observaciones, row.totalPedido, detalles, cliente));
            }
            return pedidos;
        }
        finally {
    ...
    async getById(id) {
        ...
        try {
            ...
            const [detallesRows] = await connection.execute(`SELECT d.*, pr.* 
                FROM pedido_venta_detalle d 
                INNER JOIN producto pr ON d.idproducto = pr.id 
                WHERE d.idpedidoventa = ?`, [id]);
            const detalles = detallesRows.map(det => {
                const producto = new Producto(det.idproducto, det.codigoProducto, det.denominacion, det.precioVenta);
                return new DetallePedido(det.id, det.idpedidoventa, det.idproducto, det.cantidad, det.subtotal, undefined, producto);
            });
            return new Pedido(row.pedidoId, row.idcliente, new Date(row.fechaPedido), row.nroComprobante, row.formaPago, row.observaciones, row.totalPedido, detalles, cliente);
        }
        finally {
            connection.release();
        }
    ...
    async create(pedido) {
        ...
        try {
            // Validar datos
            if (!pedido.idCliente || !pedido.detalles.length) {
                throw new Error('Datos de pedido incompletos');
            }
            const [rows] = await connection.execute(`SELECT COUNT(*) AS count 
                 FROM pedido_venta 
                 WHERE nroComprobante = ?`, [pedido.nroComprobante]);
            if (rows[0]?.count > 0) {
                throw new Error(`El número de comprobante "${pedido.nroComprobante}" ya existe.`);
            }
            // Calcular total
            pedido.totalPedido = pedido.detalles.reduce((total, detalle) => total + (detalle.cantidad * (detalle.producto?.precioVenta ?? 0)), 0);
            // Insertar pedido
            const [result] = await connection.execute(`INSERT INTO pedido_venta 
    ...
    async update(id, pedido) {
        ...
        try {
            ...
            pedido.totalPedido = pedido.detalles.reduce((total, detalle) => total + (detalle.cantidad * (detalle.producto?.precioVenta ?? 0)), 0);
            // Actualizar pedido
            await connection.execute(`UPDATE pedido_venta 
            ...
            await connection.execute('DELETE FROM pedido_venta_detalle WHERE idpedidoventa = ?', [id]);
            ...
            for (const detalle of pedido.detalles) {
                ...
                await connection.execute(`INSERT INTO pedido_venta_detalle 
                    (idpedidoventa, idproducto, cantidad, subtotal) 
                    VALUES (?, ?, ?, ?)`, [id, detalle.idProducto, detalle.cantidad, detalle.subtotal]);
            }
            await connection.commit();
    ...
    async delete(id) {
        ...
        try {
            ...
            await connection.execute('DELETE FROM pedido_venta_detalle WHERE idpedidoventa = ?', [id]);
            // Eliminar pedido
            const [result] = await connection.execute('DELETE FROM pedido_venta WHERE id = ?', [id]);
            await connection.commit();
    ...
    async getByNroComprobante(nroComprobante) {
        const connection = await Database.getConnection();
        try {
            // Obtener el pedido con datos del cliente
            const [rows] = await connection.execute(`
            ...
            const [detallesRows] = await connection.execute(`SELECT d.*, pr.* 
                FROM pedido_venta_detalle d 
                INNER JOIN producto pr ON d.idproducto = pr.id 
                WHERE d.idpedidoventa = ?`, [row.pedidoId]);
            const detalles = detallesRows.map(det => {
                const producto = new Producto(det.idproducto, det.codigoProducto, det.denominacion, det.precioVenta);
                return new DetallePedido(det.id, det.idpedidoventa, det.idproducto, det.cantidad, det.subtotal, undefined, producto);
            });
